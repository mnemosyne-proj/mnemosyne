...
class TestNonLatinFontSizeIncrease(MnemosyneTest):
    ...
    def test_1(self):
        fact_data = {"f": "question " + unichr(40960),
                     "b": "answer"}
        ...
        assert """<font style=\"font-size:14pt\">""" + unichr(40960) + "</font>" in card.question()
        
    def test_2(self):
        fact_data = {"f": "<protect>question " + unichr(40960) + "</protect>",
                     "b": "answer"}
        card_type = self.card_type_with_id("1")
        card = self.controller().create_new_cards(fact_data, card_type,
                                              grade=-1, tag_names=[])[0]
        ...
        self.config()["non_latin_font_size_increase"] = 2
        assert """<font style=\"font-size:14pt\">""" + unichr(40960) + "</font>" not in card.question()
    ...
    def test_3(self):
        ...
        self.config()["non_latin_font_size_increase"] = 2
        assert """<font style=\"font-size:14pt\">""" + unichr(40960) + "</font>" not in card.question()
        assert """<font style=\"font-size:14pt\">""" + unichr(40960) + "</font>" not in card.answer()
    ...
    def test_5(self):
        fact_data = {"f": "question " + unichr(40960) + "tail",
                     "b": "answer"}
        card_type = self.card_type_with_id("1")
        card = self.controller().create_new_cards(fact_data, card_type,
                                              grade=-1, tag_names=[])[0]
