...
class DecoratedThreeSided(CardType):
    ...
    v2.q_keys = ["m_1"]
    v2.a_keys = ["f", "p_1"]
    v2.q_key_decorators = {"m_1": "How do you say ${t}?"}
...
class TestCardType(MnemosyneTest):
    ...
    def test_database(self):
        ...
        assert card_type_out.required_keys == ["f"]
        assert card_type_out.is_data_valid({"f": "foo"}) == True

        assert card_type_out.keys == card_type.keys
        assert card_type_out.unique_keys == card_type.unique_keys
        ...
        assert card_type_out.fact_views[0].q_keys == \
               card_type.fact_views[0].q_keys
        assert card_type_out.fact_views[0].a_keys == \
               card_type.fact_views[0].a_keys
    ...
    def test_delete(self):
        ...
        assert card_type_out.fact_views[0].q_keys == \
               card_type_2.fact_views[0].q_keys
        assert card_type_out.fact_views[0].a_keys == \
               card_type_2.fact_views[0].a_keys
        assert card_type_out.fact_views[0].a_on_top_of_q == \
        ...
        assert card_type_out.fact_views[1].q_keys == \
               card_type_2.fact_views[1].q_keys
        assert card_type_out.fact_views[1].a_keys == \
               card_type_2.fact_views[1].a_keys
