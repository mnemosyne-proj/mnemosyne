...
class RenderChain(Component):
    ...
    def render_question(self, card, **render_args):
        fact_keys = card.fact_view.q_keys
        decorators = card.fact_view.q_key_decorators
        return self._render_fields(card, fact_keys, decorators, **render_args)
    
    def render_answer(self, card, **render_args):
        fact_keys = card.fact_view.a_keys
        decorators = card.fact_view.a_key_decorators
        return self._render_fields(card, fact_keys, decorators, **render_args)
    
    def _render_fields(self, card, fact_keys, decorators, **render_args):
        ...
        for fact_key in fact_keys:
            if fact_key not in data:  # Optional key.
                continue
            for filter in self._filters:
                data[fact_key] = \
                    filter.run(data[fact_key], card, fact_key, **render_args)
            if fact_key in decorators:
                data[fact_key] = \
                    string.Template(decorators[fact_key]).safe_substitute(data)
        renderer = self.renderer_for_card_type(card.card_type)
        return renderer.render_fields(data, fact_keys,
